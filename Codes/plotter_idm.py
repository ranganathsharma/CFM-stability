...
model = "idm"  # Change this to the actual model name
...
def density_perturbation_space(data: pd.DataFrame, instability: str, max_acc: float, safe_head: float):
    data = data[~data['dens'].isin([2, 142])]
...
# safe_head_array = max_acc_array = np.linspace(0.1, 4, 10)

# for max_acc in max_acc_array:
#     for safe_head in safe_head_array:

#         density_perturbation_space(data = data, instability = 'convective_numerical', max_acc = max_acc, safe_head = safe_head)

# density_perturbation_space(data = data, instability = 'convective_numerical', max_acc = 1.4, safe_head = 0.533)

# Plotting instability sequences

# a_values = [0.1, 0.1, 3.133, 0.533, 1.4, 1.833, 0.967, 4, 0.967]
# t_values = [0.1, 1.4, 0.1, 2.7, 0.533, 0.967, 1.833, 4, 2.267]

def density_perturbation_grid(
    data: pd.DataFrame,
    instability: str,
    max_acc_list: list,
    safe_head_list: list
):
    assert len(max_acc_list) == 9 and len(safe_head_list) == 9, "Both input lists must have length 9."

    # Precompute dv and round parameters
    data = data[~data['dens'].isin([2, 142])].copy()
    data['max_acc'] = data['max_acc'].round(3)
    data['safe_head'] = data['safe_head'].round(3)
    data['dv'] = (data['da'] * data['dtau']).round(3)
    data = data[data['dv'] != 25.0]

    unique_dv = sorted(data['dv'].unique())

    dv_mapping = {dv: i+1 for i, dv in enumerate(unique_dv)}

    fig, axes = plt.subplots(3, 3, figsize=(7, 6), dpi=600, sharex=True, sharey=True)

    for idx, (max_acc, safe_head) in enumerate(zip(max_acc_list, safe_head_list)):
        row, col = divmod(idx, 3)
        ax = axes[row, col]

        sub_data = data[
            (data['max_acc'] == round(max_acc, 3)) &
            (data['safe_head'] == round(safe_head, 3))
        ]

        for _, row_data in sub_data.iterrows():
            dv_mapped = dv_mapping[row_data['dv']]
            instability_val = row_data[instability]
            dens = row_data['dens']
            
            if instability_val == 1:
                ax.scatter(dens, dv_mapped, marker='s', color='b', s=5)
            elif instability_val == 2:
                ax.scatter(dens, dv_mapped, marker='s', color='r', s=5)
            elif instability_val == 3:
                ax.scatter(dens, dv_mapped, marker='s', color='g', s=5)
            elif instability_val == 0:
                ax.scatter(dens, dv_mapped, marker='o', color='m', s=5)

        if col == 0:
            ax.set_ylabel('$\\delta v$', fontsize=12)
        if row == 2:
            ax.set_xlabel('$\\rho_e$', fontsize=12)

        ax.set_title(f'$a_{{max}}$={max_acc}, $T$={safe_head}', fontsize=10)
        ax.set_xlim(0, 150)
        ax.set_xticks([0, 50, 100, 150])
        ax.set_yticks(list(dv_mapping.values()))
        ax.set_yticklabels([f"{dv:.2f}" for dv in unique_dv], fontsize=10)
        ax.set_xticklabels(ax.get_xticks(), fontsize=10)

    # Add legend outside plot
    legend_handles = [
        plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='g', markersize=6.5, label='Upstream'),
        plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='b', markersize=6.5, label='Downstream'),
        plt.Line2D([0], [0], marker='s', color='w', markerfacecolor='r', markersize=6.5, label='Absolute'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='m', markersize=6.5, label='Stability')
    ]
    fig.legend(handles=legend_handles, loc='lower center', bbox_to_anchor=(0.5, -0.05), ncol=2, fontsize=10)

    plt.tight_layout(rect=[0, 0.05, 1, 1])
    plt.savefig('density_perturbation_grid.png', bbox_inches='tight')
    plt.close()

# density_perturbation_grid(data = data,
#                           instability = 'convective_numerical',
#                           max_acc_list= a_values,
#                           safe_head_list=t_values)
...
def phase_plot(data: pd.DataFrame,
    ...
    if density == 22:
        ...
        # coordinates = [(1, 7), (1, 6), (1, 5), (1, 1), (1, 2), (2, 1), (3, 1), (4, 1), (2, 4), (2, 6), (3, 7) ]
    ...
    if density == 130:
        ...
        # coordinates = [(3, 4), (5, 1), (6, 1), (7, 1), (8, 1), (9, 1)]
        # ax = patching(ax, coordinates, 'b')
...
phase_plot(data= data, instability = 'convective_numerical', density =  22, da = 0.1, dtau = 1)
phase_plot(data= data, instability = 'convective_analytical', density =  22, da = 0.1, dtau = 1)
...
def parameter_perturbation(data: pd.DataFrame, density: float, instability: str):
    ...
    fig, axes = plt.subplots(3, 3, figsize=(6,6), dpi=600, sharex=True, sharey=True)  # 3x3 grid
    ...
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.2)  # Shift plots up to make room for legend
...
def density_variation(data, instability, max_acc, safe_head, da, dtau):
    data[['max_acc', 'safe_head']] = data[['max_acc', 'safe_head']].round(3)

    data = data[(data['max_acc'] == max_acc) & (data['safe_head'] == safe_head)]
    data = data[(data['da'] == da) & (data['dtau'] == dtau)]

    for i in range(len(data)):
        if data.iloc[i][instability] == 1:
            plt.scatter(data.iloc[i]['dens'], 1, marker='s', color='b', alpha=0.5)
        elif data.iloc[i][instability] == 2:
            plt.scatter(data.iloc[i]['dens'], 1, marker='s', color='r', alpha=0.5)
        elif data.iloc[i][instability] == 3:
            plt.scatter(data.iloc[i]['dens'], 1, marker='s', color='g', alpha=0.5)
        elif data.iloc[i][instability] == 0:
            plt.scatter(data.iloc[i]['dens'], 1, marker='s', color='gray', alpha=0.5)
    
    plt.scatter(-10, 1, marker='s', color='b', alpha=0.5, label = 'downstream instability')
    plt.scatter(-10, 1, marker='s', color='r', alpha=0.5, label = 'absolute instability')
    plt.scatter(-10, 1, marker='s', color='g', alpha=0.5, label = 'upstream instability')
    plt.scatter(-10, 1, marker='s', color='gray', alpha=0.5, label = 'Stable')
...
# density_variation(data, 'convective_analytical', max_acc = 0.1, safe_head = 0.1, da = 0.1, dtau = 1)

# Plotting the proportion of different instabilities 
def proportions(data, quantity):
    data = data[data['dens'] != 2]
    data = data[data['dens'] != 142]
...
def error_proportions(data):
    data = data[(data['dens'] != 2) & (data['dens'] != 142)]
    data['dv'] = data['da'] * data['dtau']
...
def stability_class(quantity):
    ...
    for i in range(len(data)):
        ...
        elif data.iloc[i][column_name] in ['sna', 'snua', 'snuaua', 'sndna', 'sdnau', 'sndnau','sndua', 'sndau', 'snda', 'snua', 'sndau', 'snau', 'sdau', 'snuau', 'snaua', 'snauau', 'snauauau', 'snuauau', 'sda', 'sua', 'suaua', 'suau']:
        ...
        elif data.iloc[i][column_name] in ['snaus', 'sdaus', 'sndaus', 'sdaus', 'snas', 'sdnaus', 'sndaus', 'sndas']:
...
def local_oscillations(density: float,
                        da: float,
                        dtau: float):
    ...
    for i in range(len(data)):
        temp_data = data.iloc[i]
        if temp_data['oscillations'] == 1:
            ax.scatter(temp_data['max_acc'], temp_data['safe_head'],
                        marker = 's', alpha = 1, s = 20, color = 'k')
...
# local_oscillations(density = 130, da = 0.1, dtau = 1)

def data_correction():
    data = pd.read_csv('idm_stability_status.csv')
    import copy
    temp_data = copy.deepcopy(data)

    for i in range(len(data)):
        temp = data.iloc[i]
        if (temp['string_numerical1'] + temp['string_numerical2'] > 0):
            if temp['sigma_0'] > 0:
                if temp['sigma_0'] > temp['upper_lim']:
                    if temp['convective_analytical'] == 0:
                        temp_data.at[i, 'convective_analytical'] = 2
    temp_data.to_csv(f'idm_stability_status.csv', index = False)
