...
class Stability_analysis():
    ...
    def string_stability_numerical(self) -> int:
        ...
        if np.all(delta[self.m-1:] < -self.threshold):
            stab_status = 0 # Stable
        else:
            if vel_drop[0] >= np.max(vel_drop[self.m:]):
                stab_status = 1 # Velocity drop increases
            else:
                if vel_dev[0] >= np.max(vel_dev[self.m:]):
    ...
    def string_stability_analytical(self, 
        ...
        if model == 'midm':
            ...
            term_1 = -f_dv/2*sum(self.relvel_weight[i]*(1 + i**2) for i in range(self.m))
            term_2 = 1/2/f_v**2*(-f_v*f_dv*sum(self.relvel_weight[i]*(1 + i**2) for i in range(self.m)) - 
                                 f_dv*sum(self.relvel_weight[i]*(i+1) for i in range(self.m)) - 
                                 4* f_s*(0.5 + sum(self.spacing_weight[i]*i for i in range(self.m))))
            
            term_1 = -f_dv *sum(self.relvel_weight[i]*(i+1) for i in range(self.m))
            term_2 = f_v*(0.5 + sum(self.spacing_weight[i]*i for i in range(self.m)))
            term_3 = -f_s/f_v
    ...
    def convective_string_stability_analytical(self, 
        ...
        def lambda_func(kappa):
            p = -(f_v + f_dv*(sum( self.relvel_weight[i]*(np.exp(-1j*kappa*(i+1)) - np.exp(-1j*kappa*i)) for i in range(self.m))))
            q = f_s*(1 - np.exp(-1j*kappa))*sum(self.spacing_weight[i]*np.exp(-1j*kappa*i) for i in range(self.m))
        ...
        def lambda_prime(kappa):
            ...
            pp = -f_dv*sum(self.relvel_weight[i]*(np.exp(-1j*kappa*(i+1))*(-1j*(i+1)) - np.exp(-1j*kappa*i)*(-1j*i)) for i in range(self.m))
            qp = f_s*sum((1-np.exp(-1j*kappa))*self.spacing_weight[i]*np.exp(-1j*kappa*i)*-1j*i + self.spacing_weight[i]*np.exp(-1j*kappa*i)*1j*np.exp(-1j*kappa) for i in range(self.m))
        ...
        def lambda_pprime(kappa):
            ...
            ppp = -f_dv*sum(self.relvel_weight[i]*(np.exp(-1j*kappa*(i+1))*-(i+1)**2 + np.exp(-1j*kappa*i)*i**2) for i in range(self.m))
            qpp = f_s*sum(self.spacing_weight[i]*((1 - np.exp(-1j*kappa))*np.exp(-1j*kappa*i)*(-i**2) + 
                                                  2*(np.exp(-1j*kappa*i))*i*np.exp(-1j*kappa) + 
                                                  np.exp(-1j*kappa)*np.exp(-1j*kappa*i)) for i in range(self.m))
