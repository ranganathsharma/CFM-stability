...
class Stability_analysis():
    ...
    def __init__(self, 
        ...
        self.param_list['m'] = 3
        self.param_list['weight'] = np.array([0.7, 0.2, 0.1])
        self.param_list['p'] = 0.6
        self.param_list['q'] = 0.4
        self.unpack_params()
        ...
        file_path = os.path.join(file_path, f'{model}_{dens}_stability_status_analytical.csv')
    ...
    def string_stability_numerical(self) -> int:
        ...
        self.string_numerical_value = np.max(delta[self.m-1:])
    ...
    def string_stability_analytical(self, 
        ...
        dv_ds = self.V2*self.C1*(1 - (np.tanh(self.C1*(eq_gap) - self.C2)**2))

        if (2*dv_ds - (2*self.p + 3*self.q)*self.sens_relvel)/(self.weight[0] + self.weight[1]*3 + self.weight[2]*5) < self.sens_vel:
            return 0
        else:
            return 1
    ...
    def convective_string_stability_numerical(self) -> int:
        ...
        pert_start_index = int((self.pos_array[0, int((60+ 2*self.dtau)/self.dt)]-start_pos)/dx)
        pert_end_index = int((self.pos_array[0 + self.m, int(60/self.dt)]-start_pos)/dx)
    ...
    def convective_string_stability_analytical(self, 
        ...
        def lambda_func(kappa):
            ...
            p = self.sens_vel - self.sens_relvel*(self.p*(np.exp(-1j*kappa) - 1) + self.q/2*(np.exp(-1j*kappa) + np.exp(-1j*2*kappa) - 2))
            q = -self.sens_vel*sum(self.weight[i]*(np.exp(-1j*kappa*(i+1)) - np.exp(-1j*kappa*i)) for i in range(self.m))*dv_ds

            return -p + np.sqrt(p**2 - 4*q)
        ...
        def lambda_prime(kappa):
            ...
            pp = -self.sens_relvel*(self.p*-1j*np.exp(-1j*kappa) + self.q/2*(-1j*np.exp(-1j*kappa)- 2j*np.exp(-2j*kappa)))
            qp = -self.sens_vel*sum(self.weight[i]*(-1j*(i+1)*np.exp(-1j*kappa*(i+1)) + 1j*i*np.exp(-1j*kappa*i)) for i in range(self.m))*dv_ds

            return 0.5*(-pp + (pp*p - 2*qp)/np.sqrt(p**2 - 4*q))
        ...
        def lambda_pprime(kappa):
            ...
            ppp = -self.sens_relvel*(-self.p*np.exp(-1j*kappa) + self.q/2*(-np.exp(-1j*kappa) - 4*np.exp(-2j*kappa)))
            qpp = -self.sens_vel*sum(self.weight[i]*(-(i + 1)**2*np.exp(-1j*kappa*(i+1)) + i**2*np.exp(-1j*kappa*i)) for i in range(self.m))*dv_ds

            return -ppp/2 + ((p**2 - 4*q)*(p*ppp + pp**2 - 2*qpp) - (p*pp -2*qp)**2)/2/(p**2 - 4*q)**(3/2)
    ...
    def setup_arrays(self,
        ...
        self.vel_array[:self.m,:] = self.init_vel # Boundary condition for the leader vehicle. Has to be changed based on the perturbation kind
        ...
        self.vel_array[:self.m, int(60/self.dt): int(60/self.dt) + temp_time.shape[0]] = temp_vel
    ...
    def evolve(self, 
        ...
        for count, time in enumerate(self.tim_array[:-1]):
            ...
            if np.min(x_new[:-1] - x_new[1:]) < 0:
                ...
                logging.warning(f'The vehicles have crashed for the vehicle {np.where(x_new[:-1] - x_new[1:] == np.min(x_new[:-1] - x_new[1:]))} at {count}')
                raise OverflowError()
            ...
            self.pos_array[:, count + 1], self.vel_array[self.m:, count + 1] = x_new, np.maximum(v_new[self.m:], v_new[self.m:]*0)
        
    def looper(self):
        
        num_params = 10
        sens_vel_array = np.linspace(0.2, 1, num_params)
        sens_relvel_array = np.linspace(0.2, 1, num_params)
        temp_params = self.param_list

        for i, sens_vel in enumerate(sens_vel_array):
            temp_params['sens_vel'] = sens_vel
            for j, sens_relvel in enumerate(sens_relvel_array):
                temp_params['sens_relvel'] = sens_relvel
    ...
    def processor(self, params: dict,
        ...
        temp_dict = {'n': [self.num_veh],
                    ...
                    'sens_vel': [params['sens_vel']],
                    'sens_relvel': [params['sens_relvel']],
                    'pert': [self.pert_kind], 
        ...
        if save:
            pd.DataFrame(temp_dict).to_csv(f'{self.model}_{self.init_dens}_stability_status_analytical.csv', mode = 'a', header = False, index=False)
        else:
            print(temp_dict)
            pass
