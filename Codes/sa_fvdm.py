...
class Stability_analysis():
    ...
    def string_stability_analytical(self, 
        ...
        if model == 'fvdm':
            ...
            f_dv = self.sens_relvel
            f_v = -self.sens_vel
            f_s = self.sens_vel*self.V2*self.C1*(1 - (np.tanh(self.C1*(eq_gap) - self.C2)**2))
    ...
    def convective_string_stability_analytical(self, 
        ...
        def lambda_prime(kappa):
            ...
            pp = f_dv*(1j*np.exp(-1j*kappa))
            qp = f_s*(1j*np.exp(-1j*kappa))
        ...
        def lambda_pprime(kappa):
            ...
            ppp = f_dv*np.exp(-1j*kappa)
            qpp = f_s*np.exp(-1j*kappa)

            return 0.5*(-ppp + ((p**2 - 4*q)*(ppp*p + pp**2 - 2*qpp) - (pp*p - 2*qp)**2)/(p**2 - 4*q)**(3/2))
...
if __name__ =='__main__':
    ...
    sa = Stability_analysis(model = model,
                            ...
                            da = 0.1,
                            dtau = 1)
    param_dict = model_param_dict['Models'][model]
    param_dict['sens_relvel'] = param_dict['sens_vel'] = 0.2
    sa.processor(param_dict, save = True) 